...
public class GoiMon_GUI extends JDialog {
		...
		try {
			...
			if (this.banHienTai == null) {
				...
				JOptionPane.showMessageDialog(parent,
						...
						"Lỗi Dữ liệu Bàn", JOptionPane.ERROR_MESSAGE);
				this.dispose();
				return;
			}
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(parent, "Lỗi CSDL khi lấy thông tin bàn: " + e.getMessage(), "Lỗi hệ thống",
	...
	private JPanel createMenuPanel() {
		...
		for (SanPham sp : menuMap.values()) {
			...
			JLabel lblTenMon = new JLabel(
					"<html><center><b>" + sp.getTenSP() + "</b><br>" + df.format(sp.getGia()) + "₫</center></html>");
	...
	private void hienThiChiTietDonDatBan() {
		...
		if (orderModel.getRowCount() == 0) {
			JOptionPane.showMessageDialog(this, "Vui lòng chọn món trước khi thanh toán.", "Lỗi",
					JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		try {
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(this, "Lỗi khi truy vấn Đơn Đặt Bàn: " + e.getMessage(), "Lỗi CSDL",
					JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		String maNVHienTai = "NV01";
		ChiTietHoaDonSanPham_GUI chiTietPanel = new ChiTietHoaDonSanPham_GUI(this, orderModel, isChuyenKhoan,
				this.banHienTai.getMaBan(), maNVHienTai, maKHHienTai);
	...
	class ButtonEditor extends javax.swing.AbstractCellEditor
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == editorComponent.getPlusButton()) {
				...
				currentSL--;
				if (currentSL <= 0) {
					int confirm = JOptionPane.showConfirmDialog(table, "Bạn có muốn xóa món này khỏi hóa đơn không?",
